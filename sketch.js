...
function preload(){
  ...
  backgroundImg=loadImage("BackImage.jpg");
...
function setup() {
  ...
  restart = createSprite(displayWidth/2,displayHeight/2+25);
...
function draw() {
  ...
  if (gameState===PLAY){
    ...
    if(keyDown("space") && trex.y >=displayHeight-101) {
      trex.velocityY = -15;
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    var cloud = createSprite(600,displayHeight-80,40,10);
    cloud.y = Math.round(random(500,displayHeight-200));
